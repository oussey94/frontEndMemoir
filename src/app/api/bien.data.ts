import {InMemoryDbService} from 'angular-in-memory-web-api';
import { Bien } from '../model/bien.model';

export class BienData implements InMemoryDbService {

    createDb(): Record< string, Bien[]>{
        
        const bienss : Bien[] = [
     {      
       id: 1,
       hotelName: 'Buea sweet life',
       description: 'Belle vue au bord de la mer',
       price: 230.5,
       imageUrl: 'assets/img/hotel-room.jpeg',
       rating: 3.5
    },
   {
    id: 2,
    hotelName: 'Marakech',
    description: 'Profitez de la vue sur les montagnes',
    price: 145.5,
    imageUrl: 'assets/img/the-interior.jpeg',
    rating: 5
}, 
{
  id: 3,
  hotelName: 'Abudja new look palace',
  description: 'Séjour complet avec service de voitures',
  price: 120.12,
  imageUrl: 'assets/img/indoors.jpeg',
  rating: 4
}, 
{
  id: 4,
  hotelName: 'Cape town city',
  description: 'Magnifique cadre pour votre séjour',
  price: 135.12,
  imageUrl: 'assets/img/window.jpeg',
  rating: 2.5
}
        ];

        return {bienss};
    }

    genId(bienss: Bien[]): number {
      return bienss.length >0 ? Math.max(...bienss.map(bien => bien.id)) + 1 : 1;
    }
} 